...
def load_fsspec(
    ...
    **kwargs,
    ...
    # Forzar weights_only=False para checkpoints de confianza
    if 'weights_only' not in kwargs:
        kwargs['weights_only'] = False  # ⚠️ Solo si el checkpoint es de confianza
    
    if cache and not is_local:
        with fsspec.open(
            f"filecache::{path}",
            filecache={"cache_storage": str(Path.home() / ".tts_cache")},
            mode="rb",
        ) as f:
